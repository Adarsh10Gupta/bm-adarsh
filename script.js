...
function scrollToTestimonial(index) {
  //changes for scrollable
  if (window.innerWidth >= 1024) {
    const cardWidth = testimonialCards[0].offsetWidth;
    testimonialTrack.style.transform = `translateX(-${index * cardWidth}px)`;
  } else {
    const cardWidth = testimonialCards[0].offsetWidth;
    testimonialTrack.parentElement.scrollLeft = index * cardWidth;
  }
}
...
portfolioButtons.forEach((btn) => {
  ...
  //scrollable
const cardWidth = portfolioTrack.children[0].offsetWidth;
  portfolioTrack.parentElement.scrollTo({
    left: portfolioIndex * cardWidth,
    behavior: 'smooth'
  })
});
